...
int main(int argc, char* argv[]) {
    ...
    if((ret = vinbero_Options_process(argc, argv, &config)) < VINBERO_COM_STATUS_SUCCESS) {
    ...
    if((ret = vinbero_core_initLocalModule(&module, &config)) < VINBERO_COM_STATUS_SUCCESS) {
        VINBERO_COM_LOG_ERROR("vinbero_core_initLocalModule() failed");
        return EXIT_FAILURE;
    }

    if((ret = vinbero_core_checkConfig(module.config, module.id)) < VINBERO_COM_STATUS_SUCCESS) {
        VINBERO_COM_LOG_ERROR("vinbero_core_checkConfig() failed");
        return EXIT_FAILURE;
    }

    if((ret = vinbero_core_loadChildModules(&module)) < VINBERO_COM_STATUS_SUCCESS) {
        VINBERO_COM_LOG_ERROR("vinbero_core_loadChildModules() failed");
        return EXIT_FAILURE;
    }
    if((ret = vinbero_core_initChildModules(&module)) < VINBERO_COM_STATUS_SUCCESS) {
        VINBERO_COM_LOG_ERROR("vinbero_core_initChildModules() failed");
        return EXIT_FAILURE;
    }
    if((ret = vinbero_core_rInitChildModules(&module)) < VINBERO_COM_STATUS_SUCCESS) {
    ...
    if((ret = vinbero_core_registerSignalHandlers()) < VINBERO_COM_STATUS_SUCCESS) {
