...
VINBERO_COM_MODULE_META_VERSION(VINBERO_VERSION_MAJOR,
                                VINBERO_VERSION_MINOR,
                                VINBERO_VERSION_PATCH)
...
static void
vinbero_core_sigIntHandler(int signal_number) {
    VINBERO_COM_LOG_TRACE2();
    exit(EXIT_FAILURE);
}

static void
vinbero_core_exitHandler() {
    VINBERO_COM_LOG_TRACE2();
...
vinbero_core_checkConfig(struct vinbero_com_Config* config,
    ...
    VINBERO_COM_LOG_TRACE2();
    int ret;
    if((ret = vinbero_com_Config_check(config, moduleId))
       < VINBERO_COM_STATUS_SUCCESS) {
        VINBERO_COM_LOG_ERROR("MODULE %s HAS WRONG CONFIG OR DOES NOT EXIST",
                              moduleId);
        return ret;
    }

    struct vinbero_com_Object* childModuleIds;
    if((ret = vinbero_com_Config_getChildModuleIds(config,
                                                   moduleId,
                                                   &childModuleIds))
       < VINBERO_COM_STATUS_SUCCESS)
        return ret;
    ...
    GENC_TREE_NODE_FOREACH(childModuleIds, index) {
        const struct vinbero_com_Object* childModuleId =
          GENC_TREE_NODE_RAW_GET(childModuleIds, index);
        if((ret = vinbero_core_checkConfig(
              config,
              VINBERO_COM_OBJECT_CONSTRING(childModuleId)))
           < VINBERO_COM_STATUS_SUCCESS)
            return ret;
    }
    return VINBERO_COM_STATUS_SUCCESS;
...
vinbero_core_initLocalModule(struct vinbero_com_Module* module,
    ...
    vinbero_com_Config_getInt(module->config,
                              module,
                              "vinbero.setUid",
                              (int*)&localModule->setUid,
                              (int)geteuid());
    vinbero_com_Config_getInt(module->config,
                              module,
                              "vinbero.setGid",
                              (int*)&localModule->setGid,
                              (int)getegid());
    return VINBERO_COM_STATUS_SUCCESS;
}

static int
vinbero_core_loadModule(struct vinbero_com_Module* module) {
    ...
    GENC_TREE_NODE_FOREACH(paths, index) {
        ...
        if(fastdl_open(&module->dlHandle,
                       VINBERO_COM_OBJECT_CONSTRING(path),
                       RTLD_LAZY | RTLD_GLOBAL)
           == 0) {
            module->path = VINBERO_COM_OBJECT_CONSTRING(path);
            return VINBERO_COM_STATUS_SUCCESS;
        }
        VINBERO_COM_LOG_DEBUG("FAILED TO OPEN %s FOR MODULE %s",
                              VINBERO_COM_OBJECT_CONSTRING(path),
                              module->id);
...
static bool
vinbero_core_isIfaceBigger(const char* iface, const char* iface2) {
    return strncmp(iface, iface2, strlen(iface)) > 0;
}

static bool
vinbero_core_checkIfaceGroups(const char* outIfaceGroupOriginal,
                              const char* inIfaceGroupOriginal) {
    ...
    GENC_ALIST_INIT(&inIfaceGroup);
    while((iface = strsep(&inIfaceGroupString, ",")) != NULL)
        GENC_ALIST_PUSH(&inIfaceGroup, iface);
    ...
    if(GENC_ALIST_SIZE(&outIfaceGroup) == GENC_ALIST_SIZE(&inIfaceGroup)) {
        GENC_ALIST_FOREACH(&outIfaceGroup, index) {
            if(strncmp(GENC_ALIST_RAW_GET(&outIfaceGroup, index),
...
static bool
vinbero_core_checkIfaces(const char* outIfaceGroup,
                         const char* inIfacesOriginal) {
    bool compatible = false;
    char* inIfaces = strdup(inIfacesOriginal);
    const char* inIfaceGroup;
    while(compatible == false
          && (inIfaceGroup = strsep(&inIfaces, ";")) != NULL)
...
vinbero_core_loadChildModules(struct vinbero_com_Module* module) {
    ...
    int ret;
    struct vinbero_com_Object* childModuleIds;
    if((ret = vinbero_com_Config_getChildModuleIds(module->config,
                                                   module->id,
                                                   &childModuleIds))
       < VINBERO_COM_STATUS_SUCCESS) {
        return ret;
    ...
    GENC_TREE_NODE_FOREACH(childModuleIds, index) {
        struct vinbero_com_Module* childModule =
          malloc(sizeof(struct vinbero_com_Module));
        GENC_TREE_NODE_INIT(childModule);
        GENC_TREE_NODE_ADD(module, childModule);
        childModule->id = VINBERO_COM_OBJECT_CONSTRING(
          GENC_TREE_NODE_RAW_GET(childModuleIds, index));
        childModule->config = module->config;
        ret = vinbero_core_loadModule(childModule);
        if(ret == VINBERO_COM_ERROR_INVALID_CONFIG) {
            VINBERO_COM_LOG_ERROR("CONFIG FILE IS INVALID AT MODULE %s",
                                  childModule->id);
            return ret;
        }
        if(ret == VINBERO_COM_ERROR_DLOPEN) {
            VINBERO_COM_LOG_ERROR("DLOPEN ERROR ON MODULE %s: %s",
                                  childModule->id,
                                  fastdl_error()); // dlerror is not thread safe
            return ret;
        }
        VINBERO_COM_LOG_INFO("MODULE %s LOADED FROM %s",
                             childModule->id,
                             childModule->path);
        ...
        if(ret < VINBERO_COM_STATUS_SUCCESS) {
            VINBERO_COM_LOG_ERROR("METADATA OUT_IFACES NOT FOUND ON MODULE %s",
                                  module->id);
            return VINBERO_COM_ERROR_INVALID_MODULE;
        }
        VINBERO_COM_LOG_DEBUG("OUT_IFACES OF MODULE %s IS %s",
                              module->id,
                              outIfaces);
        ...
        if(ret < VINBERO_COM_STATUS_SUCCESS) {
            VINBERO_COM_LOG_ERROR("METADATA IN_IFACES NOT FOUND ON MODULE %s",
                                  childModule->id);
            return VINBERO_COM_ERROR_INVALID_MODULE;
        }

        VINBERO_COM_LOG_DEBUG("IN_IFACES OF MODULE %s IS %s",
                              childModule->id,
                              inIfaces);
        if(vinbero_core_checkIfaces(outIfaces, inIfaces) == false) {
            VINBERO_COM_LOG_ERROR("MODULE %s AND %s ARE INCOMPATIBLE",
                                  module->id,
                                  childModule->id);
            return VINBERO_COM_ERROR_INVALID_MODULE;
        }

        if((ret = vinbero_core_loadChildModules(childModule))
           < VINBERO_COM_STATUS_SUCCESS)
            return ret;
    }

    int minChildCount = 0;
    VINBERO_COM_MODULE_META_GET(module,
                                "MIN_CHILD_COUNT",
                                &minChildCount,
                                &ret);
    if(ret < VINBERO_COM_STATUS_SUCCESS) {
    ...
    VINBERO_COM_LOG_DEBUG("MODULE %s HAS CHILD COUNT %d",
                          module->id,
                          GENC_TREE_NODE_SIZE(module));
    VINBERO_COM_LOG_DEBUG(
      "MODULE %s HAS MIN_CHILD_COUNT %d AND MAX_CHILD_COUNT %d",
      module->id,
      minChildCount,
      maxChildCount);
    if(maxChildCount != -1
       && (minChildCount > maxChildCount || minChildCount == -1)) {
        VINBERO_COM_LOG_ERROR("METADATA MIN_CHILD_COUND %d IS BIGGER THAN "
                              "MAX_CHILD_COUNT %d ON MODULE %s",
                              minChildCount,
                              maxChildCount,
                              module->id);
        return VINBERO_COM_ERROR_INVALID_MODULE;
    ...
    if(minChildCount != -1
       && minChildCount > GENC_TREE_NODE_SIZE(childModuleIds)) {
        VINBERO_COM_LOG_ERROR(
          "CHILD COUNT %d IS SMALLER THAN MIN_CHILD_COUNT %d ON MODULE %s",
          GENC_TREE_NODE_SIZE(childModuleIds),
          minChildCount,
          module->id);
        return VINBERO_COM_ERROR_INVALID_CONFIG;
    }
    if(maxChildCount != -1
       && maxChildCount < GENC_TREE_NODE_SIZE(childModuleIds)) {
        VINBERO_COM_LOG_ERROR(
          "CHILD COUNT %d IS BIGGER THAN MAX_CHILD_COUNT %d ON MODULE %s",
          GENC_TREE_NODE_SIZE(childModuleIds),
          maxChildCount,
          module->id);
        return VINBERO_COM_ERROR_INVALID_CONFIG;
...
vinbero_core_initChildModules(struct vinbero_com_Module* module) {
    ...
    GENC_TREE_NODE_FOREACH(module, index) {
        struct vinbero_com_Module* childModule =
          GENC_TREE_NODE_RAW_GET(module, index);
...
static int
vinbero_core_destroyChildModules(struct vinbero_com_Module* module) {
    VINBERO_COM_LOG_TRACE2();
    int ret;
    GENC_TREE_NODE_FOREACH(module, index) {
        struct vinbero_com_Module* childModule =
...
static int
vinbero_core_rDestroyChildModules(struct vinbero_com_Module* module) {
    VINBERO_COM_LOG_TRACE2();
    int ret;
    GENC_TREE_NODE_FOREACH(module, index) {
        struct vinbero_com_Module* childModule =
...
vinbero_core_start(struct vinbero_com_Module* module) {
    ...
    if(setjmp(*jumpBuffer) == 0) {
        ...
        GENC_TREE_NODE_FOREACH(module, index) {
            ...
            if(ret < VINBERO_COM_STATUS_SUCCESS) {
                VINBERO_COM_LOG_ERROR(
                  "FAILED TO CALL vinbero_iface_BASIC_service()");
                break;
            }
        }
    }
