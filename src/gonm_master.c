...
{
    struct gonm_worker_args* worker_args = malloc(sizeof(struct gonm_worker_args));
    ...
    if((worker_args->gonm_module_service = dlsym(dl_handle, "gonm_module_service")) == NULL)
        err(EXIT_FAILURE, "%s: %u", __FILE__, __LINE__);
    ...
    {
        if(pthread_create(threads + index, &thread_attr, gonm_worker_start, worker_args) != 0)
            err(EXIT_FAILURE, "%s: %u", __FILE__, __LINE__);
    }
