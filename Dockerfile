...
RUN mkdir /usr/src/libvinbero_common/build; cd /usr/src/libvinbero_common/build; cmake -DCMAKE_INSTALL_PREFIX:PATH=/usr -DCMAKE_BUILD_TYPE=Release ..; make; make test; make install

RUN mkdir /usr/src/vinbero-interfaces/build; cd /usr/src/vinbero-interfaces/build; cmake -DCMAKE_INSTALL_PREFIX:PATH=/usr -DCMAKE_BUILD_TYPE=Release ..; make; make test; make install
